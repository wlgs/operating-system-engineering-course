...
int main(int argc, char* argv[]){
    if (argc != 5){
        printf("Usage: n method file out_file\n");
        exit(1);
    }
    ...
    if ((strcmp(method, "numbers") != 0) && (strcmp(method, "block") != 0)) {
        printf("method are either numbers or block\n");
        exit(1);
    }
    ...
    printf("WATKI:   %d\n", threads_num);
    printf("METODA:    %s\n", method);
    fprintf(times_f, "WATKI:   %d\n", threads_num);
    fprintf(times_f, "METODA:    %s\n", method);
    for(int i = 0; i < threads_num; i++) {
        long unsigned int* t;
        pthread_join(threads[i], (void **) &t);
        printf("watek: %3d     czas: %5lu [μs]\n", i, *t);
        fprintf(times_f, "watek: %3d,     czas: %5lu [μs]\n", i, *t);
    }
    gettimeofday(&stop, NULL);
    long unsigned int* t = malloc(sizeof(long unsigned int));
    *t = (stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec;
    printf("CALKOWITY CZAS: %5lu [μs]\n", *t);
    fprintf(times_f, "CALKOWITY CZAS: %5lu [μs]\n\n\n", *t);
